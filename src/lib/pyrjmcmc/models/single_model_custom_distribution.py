# -*- coding: utf-8 -*-
"""
Created on Sat Jun 19 15:51:02 2021

@author: sebac
"""
import numpy as np

class single_order_custom_likelihood:
    ...
    def propose_birth(self, datasets, local_parameters):
        ...
        self.theta[:,new_iy] = self.nprs.rand(self.ndatasets, self.nlocal_parameters)*\
                (local_parameters[:,:,1] - local_parameters[:,:,0]) + local_parameters[:,:,0]
                
        prob = (local_parameters[:,:,1] - local_parameters[:,:,0]).prod()
    ...
    def propose_local_parameters(self, datasets, local_parameters):
        ...
        self.theta[di, self.partition_map[iy], li] = self.rs.random()*\
            (local_parameters[di,li,1]-local_parameters[di,li,0]) + local_parameters[di,li,0]
    ...
    def propose_global_parameters(self, datasets, global_parameters):
        ...
        self.global_parameters[di,gi] = self.rs.random()*\
            (global_parameters[di,gi,1] - global_parameters[di,gi,0]) + global_parameters[di,gi,0]
