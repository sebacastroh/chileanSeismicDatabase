# -*- coding: utf-8 -*-
"""
Created on Sun Jun 27 14:14:49 2021
...
class mp_mo_goh:
    ...
    def __init__(self, npoints, ndatasets, xmin, xmax, min_partitions, max_partitions,
        ...
        # Model values
        self.theta_g = np.empty((ndatasets, nglobal_parameters))
    ...
    def initialize(self, datasets):
        ...
        if self.computeDesignMatrices:
            for di, dataset in enumerate(datasets):
                for ki in range(self.kmax):
                    ...
                    if self.nglobal_parameters > 0:
                        self.X[di][ki,:,:nl] = self.design_matrices(dataset[:,0], self.theta_g[di], self.boundaries, ki)
                    else:
                        self.X[di][ki,:,:nl] = self.design_matrices(dataset[:,0], self.boundaries, ki)
        else:
        ...
        for i, pi in enumerate(self.partition_map):
            ...
            for di, dataset in enumerate(datasets):
                ...
                if self.nglobal_parameters > 0:
                    self.lf_values[di][:,pi] = self.local_function(dataset[:,0], self.theta_g[di], xl, xr, ki, self.theta[di][pi,:nl])
                else:
                    self.lf_values[di][:,pi] = self.local_function(dataset[:,0], xl, xr, ki, self.theta[di][pi,:nl])
    
    #TODO chequear funcion
    ...
    def _propose_global_parameters(self, datasets):
        ...
        if self.computeDesignMatrices:
            for ki in range(self.kmax):
                nl = self.nlocal_parameters[ki]
                if self.nglobal_parameters > 0:
                    self.X[di][ki,:,:nl] = self.design_matrices(datasets[di][:,0], self.theta_g[di], self.boundaries, ki)
                else:
                    self.X[di][ki,:,:nl] = self.design_matrices(datasets[di][:,0], self.boundaries, ki)
    ...
    def sample(self, x, di):
        if self.nglobal_parameters > 0:
            return self.sampling_function(x, self.theta_g[di], self.boundaries,
                                          self.k[di][self.partition_map], self.theta[di][self.partition_map],
                                          self.nlocal_parameters)
        else:
            return self.sampling_function(x, self.boundaries, self.k[di][self.partition_map],
                                          self.theta[di][self.partition_map], self.nlocal_parameters)
