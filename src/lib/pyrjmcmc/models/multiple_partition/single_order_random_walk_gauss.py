...
class mp_so_rwg:
    ...
    def _propose_birth(self, datasets):
        ...
        for di in range(self.ndatasets):
            dv = self.nprs.normal(scale=self.local_parameters[di,:,2])
            self.theta[di,new_iy] += dv
                
            if np.any(self.theta[di,new_iy] < self.local_parameters[di,:,0]) or \
                np.any(self.theta[di,new_iy] > self.local_parameters[di,:,1]):
                return 0, 0.
            
            normal_prob = spsts.norm.pdf(dv, scale=self.local_parameters[di,:,2]).prod()
            prob /= normal_prob*self.delta_prod
    ...
    def _propose_death(self, datasets):
        ...
        for di in range(self.ndatasets):
            dv = self.theta[di,del_iy] - self.theta[di,self.partition_map[prev_i]]
    ...
    def _propose_merge(self, datasets):
        ...
        for di in range(self.ndatasets):
            dv = self.theta[di,jy] - self.theta[di,self.partition_map[prev_i]]
    ...
    def _propose_local_parameters(self, datasets):
        ...
        dv = self.nprs.normal(0., self.local_parameters[di,li,2])
        
        self.theta[di,pi,li] += dv
        
        if self.theta[di,pi,li] < self.local_parameters[di,li,0] or\
           self.theta[di,pi,li] > self.local_parameters[di,li,1]:
    ...
    def _propose_global_parameters(self, datasets):
        ...
        dv = self.nprs.normal(0., self.global_parameters[di,gi,2])
        
        self.theta_g[di][gi] += dv
